...
def apply(state, cfg, prices, ds, *rest):
    """engine.apply_decisions with a passing default plan added to buys that lack one."""
    ds = [dict(d) for d in ds]
    for d in ds:
        r = prices.get(d.get("ticker")) or {}
        if d.get("action") == "buy" and "target" not in d and r.get("ok"):
            d.update(target=r["price"] * 1.15, stop=r["price"] * 0.95, prob=0.6)
    return engine.apply_decisions(state, cfg, prices, ds, *rest)
...
def test_cash_buffer_kept():
    ...
    s["cash"] = 6000  # NAV 10k
    apply(s, US, {"AAPL": P(10)}, [{"action": "buy", "ticker": "AAPL", "usd": 5000}], "t", "d", False)
    assert s["cash"] >= 0.05 * 10000 - 1e-6


def test_unknown_ticker_and_missing_price_rejected():
    s = fresh()
    ex, rej = apply(s, US, {"AAPL": {"ok": False}}, [
...
def test_daytrade_rules_and_flatten():
    s = fresh(100000)
    ex, _ = apply(s, US, {"AMD": P(100)}, [{"action": "buy", "ticker": "AMD", "bucket": "daytrade", "usd": 50000}], "t", "d1", False)
    assert s["positions"]["AMD"]["shares"] == 100  # 10% cap
    ex, _ = apply(s, US, {"NVDA": P(100)}, [{"action": "buy", "ticker": "NVDA", "bucket": "daytrade", "usd": 5000}], "t", "d1", True)
...
def test_overnight_daytrade_is_flattened_next_day():
    s = fresh(100000)
    apply(s, US, {"AMD": P(100)}, [{"action": "buy", "ticker": "AMD", "bucket": "daytrade", "usd": 5000}], "t", "d1", False)
...
def test_sell_more_than_held_and_partial():
    ...
    ex, _ = apply(s, US, {"AAPL": P(120)}, [{"action": "sell", "ticker": "AAPL", "shares": 4}], "t", "d", False)
    assert s["positions"]["AAPL"]["shares"] == 6 and ex[0]["realizedPnL"] == 80.0
    ex, _ = apply(s, US, {"AAPL": P(120)}, [{"action": "sell", "ticker": "AAPL", "shares": "all"}], "t", "d", False)
    assert "AAPL" not in s["positions"]


def test_crypto_fractional_units():
    s = fresh(45000)
    apply(s, CRYPTO, {"BTC-USD": P(84000)}, [{"action": "buy", "ticker": "BTC-USD", "usd": 5000}], "t", "d", False)
    assert 0.05 < s["positions"]["BTC-USD"]["shares"] < 0.06


def test_average_cost_on_add():
    s = fresh(100000)
    apply(s, US, {"AAPL": P(100)}, [{"action": "buy", "ticker": "AAPL", "usd": 5000}], "t", "d", False)
    apply(s, US, {"AAPL": P(120)}, [{"action": "buy", "ticker": "AAPL", "usd": 6000}], "t", "d", False)
    p = s["positions"]["AAPL"]
...
def test_costs_reduce_cash_and_raise_cost_basis():
    s = fresh(100000)
    ex, _ = apply(s, US_REAL, {"AAPL": P(100)}, [{"action": "buy", "ticker": "AAPL", "usd": 10000}], "t", "d", False)
    t = ex[0]
...
def test_round_trip_at_flat_price_loses_money():
    for cfg, price, usd in ((US_REAL, 100, 10000), (ASX_REAL, 50, 10000), (CRYPTO_REAL, 84000, 10000)):
        ...
        apply(s, cfg, {key: P(price)}, [{"action": "buy", "ticker": key, "usd": usd}], "t", "d", False)
        ex, _ = apply(s, cfg, {key: P(price)}, [{"action": "sell", "ticker": key, "shares": "all"}], "t", "d", False)
